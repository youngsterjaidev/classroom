...
class AssignmentInvitation < ApplicationRecord
  ...
  def reason_for_disabled_invitations
    return AssignmentRepo::Creator::Result.failed(INVITATIONS_DISABLED) unless assignment.invitations_enabled?
    return AssignmentRepo::Creator::Result.failed(INVITATIONS_DISABLED_ARCHIVED) if assignment.organization.archived?
  end
end
